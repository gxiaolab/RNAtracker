...
deal_with_zeroes = function(sum.of.comps, comp0.post, comp1.post, comp0_df, comp1_df){
    sum.of.comps.ln <- log(sum.of.comps, base = exp(1))

    # 0 exists in sum.of.comps
    if (sum(sum.of.comps == 0) > 0){
        small_prob_gene_id <- which(sum.of.comps == 0)
        small_post_mat <- matrix(0, nrow=length(small_prob_gene_id), ncol=2)
        for (cur_small_prob_gene_id in small_prob_gene_id){
            loggp_vec_prec <- c(exp(Rmpfr::mpfr(comp0_df$logP[cur_small_prob_gene_id],prec=40)), exp(Rmpfr::mpfr(comp1_df$logP[cur_small_prob_gene_id],prec=40)))
    ...
    posterior.df <- data.frame(gene_id=comp0_df[,1], comp0.post, comp1.post)
...
e_step <- function(x, n, gene_ind_list, pi, a0, b0, a1, b1){
    ...
    comp0.dens <- c(dbetabinom(x_time0, n_time0, prob=mu0, rho=rho0))
    comp1.dens <- c(dbetabinom(x_time0, n_time0, prob=mu1, rho=rho1))
    ...
    sum.of.comps <- pi[1] * comp0_df$product + pi[2] * comp1_df$product 
  
    comp0.post <- pi[1] * comp0_df$product / sum.of.comps
    comp1.post <- pi[2] * comp1_df$product / sum.of.comps

    out = deal_with_zeroes(sum.of.comps, comp0.post, comp1.post, comp0_df, comp1_df)
...
q_function <- function(x, n, a0, b0, a1, b1, gene_ind_list, posterior.df){
    ...
    comp0.log <- c(dbetabinom(x_time0, n_time0, prob=mu0, rho=rho0, log=TRUE))
    comp1.log <- c(dbetabinom(x_time0, n_time0, prob=mu1, rho=rho1, log=TRUE))
    # Wrong here
    comp0.log.sum <- sum(complete.posterior.df[,1] * comp0.log)
    comp1.log.sum <- sum(complete.posterior.df[,2] * comp1.log)


    return(Q=  comp0.log.sum + comp1.log.sum )
