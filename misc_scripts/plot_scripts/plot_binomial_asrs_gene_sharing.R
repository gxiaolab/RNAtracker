...
out = fread('../results/asrs_celltype_sharing_info.combos.txt')
...
p = ggplot(all_dat, aes(fill = `Proportion Type`, y = Proportion, x = label)) + geom_text(data = all_dat %>% filter(`Proportion Type` == 'Actual'), aes(label = pstar, x = label, y = Proportion), nudge_y = 0.0015, size = 3) + 
...
write.table(sig_binom %>% select(-hyper_pval_vec), file = '../../../SOURCE_DATA/MAIN_FIGS/fig1e.txt', sep = '\t', row.names = F, quote = F)

pdf('../figures/asrs_celltype_sharing_binom_sig.pdf')
p
dev.off()


pdf('../MANUSCRIPT_FIGURES/fig1/fig1e.pdf', width = 3, height = 3)
